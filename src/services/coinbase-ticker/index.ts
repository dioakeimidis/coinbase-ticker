...
import { SubscribeAsset, UnsubscribeAsset, ProductIds, TickerModel } from "./types";

export class CoinbaseTicket extends EventEmitter {
  static assetIds: ProductIds[] = ["BTC-USD", "ETH-USD", "LTC-USD"];
  ...
  connect() {
    ...
    this.#ws.onopen = () => {
      ...
      for (let asset of CoinbaseTicket.assetIds) {
    ...
    this.#ws.onerror = (error: Event) => {
      ...
      if (this.#ws?.readyState === WebSocket.OPEN || this.#ws?.readyState === WebSocket.CONNECTING) {
        this.#ws.close(1001, "Reconnecting"); // Code 1001: Going Away.
      }
